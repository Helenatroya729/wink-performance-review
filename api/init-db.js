  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS potential_assessments (
        ...
        development_desire VARCHAR(20) CHECK (development_desire IN ('proactive', 'needs_help', 'not_sure', 'no')),
        is_successor BOOLEAN DEFAULT FALSE,
        successor_ready_timing VARCHAR(20) CHECK (successor_ready_timing IN ('now', '6_months', 'not_ready')),
        turnover_risk INTEGER CHECK (turnover_risk >= 0 AND turnover_risk <= 10),
        ole_priority_1 TEXT,
        ole_priority_2 TEXT,
        performance_raw_score INTEGER DEFAULT 0,
        performance_final_score INTEGER DEFAULT 1,
        potential_raw_score INTEGER DEFAULT 0,
        potential_final_score INTEGER DEFAULT 1,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    if (parseInt(usersCheck.rows[0].count) === 0) {
      ...
      await client.query(`
        ...
        VALUES ($1, $2, 'Анна', 'HR', 'hr', 'HR Manager', 'Управление персоналом')
      `, ['hr@wink.ru', hashedPassword]);

      // Менеджер
      await client.query(`
        INSERT INTO users (email, password, first_name, last_name, role, position, department)
        VALUES ($1, $2, 'Кирилл', 'Менеджеров', 'manager', 'Team Lead', 'Разработка')
      `, ['manager1@wink.ru', hashedPassword]);

      // Сотрудник
      const managerResult = await client.query('SELECT id FROM users WHERE email = $1', ['manager1@wink.ru']);
      const managerId = managerResult.rows[0].id;

      await client.query(`
        INSERT INTO users (email, password, first_name, last_name, role, position, department, manager_id)
        VALUES ($1, $2, 'Иван', 'Сотрудников', 'employee', 'Developer', 'Разработка', $3)
      `, ['emp1@wink.ru', hashedPassword, managerId]);
    ...
    if (parseInt(cyclesCheck.rows[0].count) === 0) {
      await client.query(`
        ...
        VALUES ('Q4 2024', '2024-10-01', '2024-12-31', 'active')
      `);
      console.log('✅ Цикл оценки создан!');
    } else {
      console.log('ℹ️  Цикл оценки уже существует');
    }

    console.log('\n🎉 База данных успешно инициализирована!');
    console.log('\n📝 Тестовые аккаунты:');
    console.log('   HR: hr@wink.ru / 123456');
    console.log('   Менеджер: manager1@wink.ru / 123456');
    console.log('   Сотрудник: emp1@wink.ru / 123456\n');
